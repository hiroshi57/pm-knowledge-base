...
flowchart TB
  E["出来事：締め処理で止まった"]
  E --> SL["シングルループ：例外の洗い出しを徹底（やり方を直す）"]
  E --> DL["ダブルループ：経理を外した判断は誰がいつした？（前提を問う）"]
```
...
```mermaid
flowchart TB
  A["改修が承認される（審査に平均3週間）"] --> X["全部の掛け算が成立する姿は想像しにくい"]
  B["改修が2週で影響ゼロ（やってみないと分からない）"] --> X
  C["本部長が展開に合意（会話が始まっていない）"] --> X
  D["20店舗の教育が終わる（標準手順がまだない）"] --> X
  X --> Y["三択：縮小 / 延期 / 中止"]
```
