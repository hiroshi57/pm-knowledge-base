...
```
 予測型 ◀──────────────── ハイブリッド ────────────────▶ 適応型
 (Waterfall)                  (実態の多く)                 (Agile)
 要件が安定/規制が厳しい      予算・契約=予測型             要件が変化/市場検証
 大規模インフラ              機能開発=適応型              プロダクト開発
   │                                                        │
   └── 選ぶ基準＝間違えたときのコストと変更頻度（原則1＋テーラリング）──┘
```

**図：ハイブリッドは「1プロジェクトの中で線を引く」**
```
  同じプロジェクト
  ├─ 予測型で固定（変更コスト高）：全体予算 / 法規制 / 契約 / リリース期日
  └─ 適応型で回す（変更歓迎）    ：機能詳細 / UI・UX / 実装順序
     どちらに属するかを"事前に"合意 → 「なぜ今さら仕様変更を止めるのか」を防ぐ
```
