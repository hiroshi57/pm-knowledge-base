...
|------|-------------------|---------------------|---------------------|----------|
| プロセス | 49プロセスとして規定 | 明示的な規定なし（原則・パフォーマンス領域中心） | 40プロセスとして復活・再統合（要検証、[04](standard/04_process_reference.md)） | 「何をどの順でやるか」の実行単位 |
| 原理原則 | 明示的なリストなし（暗黙的に各章に内在） | 12原則として明文化 | 6原則に再編（[02](standard/02_six_principles.md)） | 「どう判断し、どう振る舞うか」の指針 |
| パフォーマンス領域 | 概念として存在せず（知識エリアが近い） | 8領域として明文化 | 7領域に再編（要検証、[03](standard/03_performance_domains.md)） | 「継続的に見るべき活動の切り口」 |
...
| 成果の成功／プロセスの成功 | 明示的な区別は薄い（QCD中心） | 価値提供の重視という形で片鱗あり | 二軸評価として明確化（[05](standard/05_success_criteria.md)） | プロジェクト評価の二本柱 |
| 現代的テーマ（Contemporary Themes） | なし | 付録的な扱い | 本編テーマとしてAI等を明記（[06](standard/06_ai_contemporary_theme.md)） | 継続的に変化する社会的トレンドへの対応 |
...
| MVP（Minimum Viable Product） | 価値仮説を検証するために必要な最小限の機能を備えた製品。[原則2：価値に注力する](standard/02_six_principles.md)と直結 |
| Definition of Done（完了の定義） | 成果物が「完了」とみなせる条件の合意事項。[原則3：品質を組み込む](standard/02_six_principles.md)の実践手段の一つ |
| ベロシティ（Velocity） | アジャイルチームが1スプリントあたりに完了させられる作業量の実績値 |
| バーンダウン／バーンアップチャート | 残作業量／完了作業量の推移を可視化するグラフ。適応型プロジェクトの進捗管理に使う |
| プログラム（Program） | 一括管理した方が便益が大きい、関連する複数プロジェクトの集合（[01.6](standard/01_overview_history.md)参照） |
| ポートフォリオ（Portfolio） | 戦略目標達成のために一括管理される、プロジェクト・プログラム・業務の集合（[01.6](standard/01_overview_history.md)参照） |
...
- 全体像・版の変遷 → [01_overview_history.md](standard/01_overview_history.md)
- 6原理原則の詳細 → [02_six_principles.md](standard/02_six_principles.md)
- パフォーマンス領域 → [03_performance_domains.md](standard/03_performance_domains.md)
...
- PMBOK↔内製教科書の対応と統合判断 → [CROSSWALK.md](CROSSWALK.md)
