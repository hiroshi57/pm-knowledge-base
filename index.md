...
hero:
  ...
  text: PMBOK第8版 × 内製チーム向け実践
  tagline: 標準の型で語り、内製の判断で動く。2つの資料をマージした統合ナレッジベース。
  actions:
    - theme: brand
      text: 標準層(PMBOK)を見る
...
features:
  - title: 標準層 — PMBOK第8版
    details: 6つの原理原則・パフォーマンス領域・成功の二軸・AIテーマなど、どの現場でも通じる「型・用語・体系」。
    link: /standard/02_six_principles
    linkText: 6つの原理原則へ
...
- **標準層（PMBOK第8版）** … 研修キット。標準として通じる型・用語・体系。
- **実践層（内製教科書）** … 『プロジェクトマネジメントの教科書（内製チーム向け）』全42章。
- **統合層** … 両者で重複していた6ツールを、良い方を残して統合。判断は [CROSSWALK](/CROSSWALK) に記録。

> 一次情報の注記：PMBOK第8版の一部（7領域名称・40プロセス詳細）は公式ガイド未照合の `要検証` を含む。研修投入前に照合すること（[要検証タスク](/task-lists)）。
